    ...
    def eval_rho(self, parameters, rho):
        ...
        for Omega, (A, B) in zip(parameters, self.operators):
            if i % 2 == 0:
                ...
                rho = np.exp(-1j * (Omega * np.diag(A.todense()) + B)) * rho
            i += 1
        return rho
...
class JCMeasCircuit(Circuit):
    ...
    def __init__(self, setup, layers_p, layers_m, delta):
        ...
        self.setup = setup
        self.operators = self.layers_p*[(0*sp.eye(setup.d, dtype=np.complex128),
         setup.H_c()), (setup.H_e(), 0), (setup.H_t(), 0), (setup.H_int(), 0)]
        ...
        self.delta = delta
        self.operators_meas = self.layers_m*[(0*sp.eye(setup.d, dtype=np.complex128),
         setup.H_c()), (setup.H_e(), 0), (setup.H_t(), 0), (setup.H_int(), 0)]
    ...
    def measurement_cfi(self, parameters, psi, grad_psi):
        ...
        grad_rho_c = 0 # Initialize reduced density matrix of cavities
        for i in range(2**self.setup.N_e):
            grad_rho_c += sp.kron(self.basis_e[:, i].H, self.basis_c) @ grad_rho @ sp.kron(self.basis_e[:, i], self.basis_c)

        # Evaluate classical Fisher information
        I = np.sum(1/np.diag(rho_c + 1e-30) * np.diag(grad_rho_c)**2)
...
class KerrMeasCircuit(Circuit):
    ...
    def __init__(self, setup, layers_p, layers_m, delta):
        ...
        self.setup = setup
        self.N_p = setup.N_p
        self.operators = self.layers_p*[(setup.H_BS(), 0), (setup.H_k(), 0)]
        ...
        self.delta = delta
        self.operators_meas = self.layers_m*[(setup.H_BS(), 0), (setup.H_k(), 0)]

        # Quadrature operators
        X = 1/sqrt(2) * (create(setup.N_p+1) + destroy(setup.N_p+1))
        Xs = X.eigenstates()[1]
        self.X_ij = np.zeros((setup.N_p+1, setup.N_p+1, (setup.N_p+1)**2, 1), dtype=np.complex128)
        for i, x_i in enumerate(Xs):
            for j, x_j in enumerate(Xs):
                self.X_ij[i, j] = tensor(x_i, x_j)
    ...
    def measurement_cfi(self, parameters, psi, grad_psi):
        ...
        # Probabilities
        Prob_ij = np.abs(np.einsum('ijkl,lm->ijkm', np.reshape(self.X_ij, (self.N_p+1, self.N_p+1, 1, (self.N_p+1)**2)).conj(), psi[:, np.newaxis]))**2 + 1e-30

        # Derivatives
        grad_Prob_ij = np.einsum('ijkl,lm->ijkm', np.reshape(self.X_ij, (self.N_p+1, self.N_p+1, 1, (self.N_p+1)**2)).conj(), grad_psi[:, np.newaxis])\
        * np.einsum('kl,ijlm->ijkm', psi[np.newaxis, :].conj(), self.X_ij)\
        + np.einsum('ijkl,lm->ijkm', np.reshape(self.X_ij, (self.N_p+1, self.N_p+1, 1, (self.N_p+1)**2)).conj(), psi[:, np.newaxis])\
        * np.einsum('kl,ijlm->ijkm', grad_psi[np.newaxis, :].conj(), self.X_ij)

        I = np.sum(np.sum(1/Prob_ij * grad_Prob_ij**2))
...
class Setup:
    ...
    def __init__(self, N_e, N_c, N_p, k):
        ...
        self.k = k # Exponent of the photon-photon interaction
    ...
    def H_k(self):
        'Generator of an interaction of order k.'

        H = 0
        for i in range(self.N_c):
            H += (self.a_c(i).H @ self.a_c(i))**self.k
