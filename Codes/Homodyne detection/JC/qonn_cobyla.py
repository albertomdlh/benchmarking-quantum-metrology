...
class JCMeasCircuit(Circuit):
    ...
    def __init__(self, setup, layers_p, layers_m, delta):
        ...
        X = 1/sqrt(2) * (create(setup.N_p+1) + destroy(setup.N_p+1))
        Xs = X.eigenstates()[1]
        self.X_ijkl = np.zeros((2, 2, setup.N_p+1, setup.N_p+1, 4*(setup.N_p+1)**2, 1), dtype=np.complex128)
        for i in range(2):
            for j in range(2):
                for k, x_k in enumerate(Xs):
                    for l, x_l in enumerate(Xs):
                        self.X_ijkl[i, j, k, l] = tensor(basis(2, i), basis(2, j), x_k, x_l)
    ...
    def measurement_cfi(self, parameters, psi, grad_psi):
        ...
        # Probabilities
        Prob_ij = np.abs(np.einsum('ijklmn,no->ijklmo', np.reshape(self.X_ijkl, (2, 2, self.N_p+1, self.N_p+1, 1, 4*(self.N_p+1)**2)).conj(), psi[:, np.newaxis]))**2 + 1e-30

        # Derivatives
        grad_Prob_ij = np.einsum('ijklmn,no->ijklmo', np.reshape(self.X_ijkl, (2, 2, self.N_p+1, self.N_p+1, 1, 4*(self.N_p+1)**2)).conj(), grad_psi[:, np.newaxis])\
        * np.einsum('mn,ijklno->ijklmo', psi[np.newaxis, :].conj(), self.X_ijkl)\
        + np.einsum('ijklmn,no->ijklmo', np.reshape(self.X_ijkl, (2, 2, self.N_p+1, self.N_p+1, 1, 4*(self.N_p+1)**2)).conj(), psi[:, np.newaxis])\
        * np.einsum('mn,ijklno->ijklmo', grad_psi[np.newaxis, :].conj(), self.X_ijkl)

        I = np.sum(np.sum(np.sum(np.sum(1/Prob_ij * grad_Prob_ij**2))))

        return - np.real(I)

class KerrMeasCircuit(Circuit):

    def __init__(self, setup, layers, gamma):
        super().__init__
        self.layers = layers
        self.setup = setup
        self.operators = self.layers*[(setup.H_BS_vec(), 0), (setup.H_Kerr_vec(), 0)]
        self.operators_interf = [(0*sp.eye(setup.d_c**2, dtype=np.complex128), setup.H_BS_sym_vec()),
         (setup.H_phi_vec(), 0),
         (0*sp.eye(setup.d_c**2, dtype=np.complex128), setup.H_BS_sym_vec())]
        L = 0.0
        for k in range(setup.N_c):
            L += gamma*(sp.kron(setup.a_c(k).conj(), setup.a_c(k)) - 0.5*sp.kron(sp.eye(setup.d_c, dtype=np.complex128),
                setup.a_c(k).H @ setup.a_c(k)) - 0.5*sp.kron((setup.a_c(k).H @ setup.a_c(k)).transpose(), sp.eye(setup.d_c, dtype=np.complex128)))
        self.L = L

    def interf_qfi(self, parameters, phi, rho):

        # Preparation VQC
        rho = self.eval_rho(parameters[:2*self.layers], rho)

        # Encoding (MZ interferometer)
        rho, grad_rho = self.eval_rho_interf(phi, rho)

        # Losses
        rho = sp.linalg.expm_multiply(self.L, rho)
        grad_rho = sp.linalg.expm_multiply(self.L, grad_rho)

        # Measurement VQC
        rho = self.eval_rho(parameters[2*self.layers:], rho)
        grad_rho = self.eval_rho(parameters[2*self.layers:], grad_rho)
